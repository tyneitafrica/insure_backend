...
urlpatterns = [
    ...
    path("api/v1.0/motorinsurance/users/quotes/", GetMotorInsuranceUsers.as_view(), name="organisation filtered users"), #to be used to get users who had enquired about a quote


# #    upload marine section 
    path("api/v1.0/applicant/marine/quote/", MarineInsuranceApplicationView.as_view(), name="applicant creates marine insurance quote"),
    path("api/v1.0/marineinsurance/pdf/<int:id>/", GenerateMarineInsurancePDF.as_view(), name="download marine insurance pdf "),  
...
#     path("api/v1.0/marineinsurance/<int:id>/", UpdateMarineInsurance.as_view(), name="edit marine insurance"), #will handle the patch and delete of marine insurance
    ...
    path("api/v1.0/mpesa-status/<str:id>/", HandlePaymentStatus.as_view(), name="pay status")
